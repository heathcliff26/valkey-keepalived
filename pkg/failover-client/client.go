	...
	for {
		...
		if currentMaster != c.currentMaster {
			...
			if !found {
				...
				slog.Info("Switching over to new master", slog.String("addr", c.masterNode.address), slog.Int64("port", c.masterNode.port), slog.String(runID, c.currentMaster))
			}
		}
